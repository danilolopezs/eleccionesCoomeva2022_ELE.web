...
public class LogicaReportes {
	 ...
	 * @throws Exception
	 */
	public OutputStream generarAsociados(List<AsociadoReporteDTO> listaAsociados, String pathServidor)
			throws Exception {
	...
	public OutputStream generarTransacciones(List<LogTransacciones> listaTransacciones, String pathServidor) {
		Document reporteAsociado = null;
		ByteArrayOutputStream file = null;
		HeaderFooter header = null;
		try {
			...
			for (LogTransacciones log : listaTransacciones) {
				...
				String antHab = "";
				if (log.getAnthabil().toString().trim()
						.equalsIgnoreCase(UtilAcceso.getParametroFuenteS("parametros", "caracterEstadoAsociado1"))) {
	...
	public OutputStream generarTransaccionesPlanLog(List<EleLog> listaTransacciones, String pathServidor,
			...
			for (EleLog log : listaTransacciones) {
				...
				if (log.getUsuario().equalsIgnoreCase("Asociado")) {
					...
					EleCabPlancha eleCabPlancha = DelegadoCabezaPlancha.getInstance()
							.consultarCabezaPlancha(log.getNroCaboPlancha());
					...
					if (eleCabPlancha != null) {
						...
						nombreCompleto = primerNombre + " " + segundoNombre + " " + primerApellido + " "
								+ segundoApellido;
					}
				} else
	...
	public ByteArrayOutputStream generarPlanchas(List<ElePlanchas> listaPlanchas, String pathServerContextPath)
			throws Exception {
		...
		try {
			...
			for (ElePlanchas plancha : listaPlanchas) {
				...
				if (plancha.getEstado()
						.equalsIgnoreCase(UtilAcceso.getParametroFuenteS("parametros", "estadoPlancha3"))) {
					table.addCell(UtilAcceso.getParametroFuenteS("parametros", "displayEstado3"));
					flag = true;
				}

				if (plancha.getEstado()
						.equalsIgnoreCase(UtilAcceso.getParametroFuenteS("parametros", "estadoPlancha2"))) {
					table.addCell(UtilAcceso.getParametroFuenteS("parametros", "displayEstado2"));
					flag = true;
				}

				if (plancha.getEstado()
						.equalsIgnoreCase(UtilAcceso.getParametroFuenteS("parametros", "estadoPlancha7"))) {
					table.addCell(UtilAcceso.getParametroFuenteS("parametros", "displayEstado7"));
					flag = true;
				}

				if (plancha.getEstado()
						.equalsIgnoreCase(UtilAcceso.getParametroFuenteS("parametros", "estadoPlancha8"))) {
					table.addCell(UtilAcceso.getParametroFuenteS("parametros", "displayEstado8"));
					flag = true;
				}

				if (plancha.getEstado()
						.equalsIgnoreCase(UtilAcceso.getParametroFuenteS("parametros", "estadoPlancha9"))) {
...
//			reporteAsociado.setMargins(80, 60, 50.0f, 40.f);
			...
			EleZonasFinanciero zonaFin = LogicaZonaFinanciera.getInstance()
					.consultarZonaFinanciero(asociadoDTO.getOficina());
			...
			cell = new PdfPCell(
					new Paragraph(UtilAcceso.getParametroFuenteS("parametros", "hdrttlPrincipales"), fontBold));
...
//			cell.setBorderWidthTop(2);
			...
			tablePrincipal.addCell(cell);
			cell = new PdfPCell(
					new Paragraph(UtilAcceso.getParametroFuenteS("parametros", "hdrNombreApellidos"), font));
			cell.setBackgroundColor(HdColor);
			cell.setBorderColor(Color.decode("#009933"));
			cell.setHorizontalAlignment(Element.ALIGN_CENTER);
			tablePrincipal.addCell(cell);
			...
			tablePrincipal.addCell(cell);
			cell = new PdfPCell(
					new Paragraph(UtilAcceso.getParametroFuenteS("parametros", "hdrCedulaCiudadania"), font));
			cell.setBackgroundColor(HdColor);
			cell.setBorderColor(Color.decode("#009933"));
			cell.setHorizontalAlignment(Element.ALIGN_CENTER);
			tablePrincipal.addCell(cell);
			...
			cell = new PdfPCell(
					new Paragraph(UtilAcceso.getParametroFuenteS("parametros", "hdrttlSuplentes"), fontBold));
			cell.setBackgroundColor(Color.decode("#99CCCC"));
			cell.setHorizontalAlignment(Element.ALIGN_CENTER);
			cell.setColspan(5);
			cell.setBorderColor(Color.decode("#009933"));
			...
			cell = new PdfPCell(
					new Paragraph(UtilAcceso.getParametroFuenteS("parametros", "pdfNombreInsPlancha"), font));
			cell.setBorder(0);
			...
			tableNomIns.addCell(cell);
			cell = new PdfPCell(
					new Paragraph(UtilAcceso.getParametroFuenteS("parametros", "pdfFirmaInsPlancha"), font));
			cell.setBorder(0);
			tableNomIns.addCell(cell);
			cell = new PdfPCell(new Paragraph("", font));
			cell.setBorderColor(Color.decode("#009933"));
			...
			String[] otrosEstudios = plancha.getEleCabPlanchaDTO().getOtrosEstudios()
					.split(UtilAcceso.getParametroFuenteS("parametros", "separadorEstudios"));
			...
			if (!numeroPlancha.trim().equalsIgnoreCase("")) {
				imagenCabezaPlancha = pathContext + "//" + "imagenes" + "//" + "asociados" + "//" + zonas + "_"
						+ numeroPlancha + "_" + nombreAsociado + ".jpg";
			} else
				imagenCabezaPlancha = pathContext + "//" + "imagenes" + "//" + "asociados" + "//" + zonas + "_"
						+ nombreAsociado + ".jpg";

			if (plancha.getEleCabPlanchaDTO().getRutaImagen() != null
					&& !plancha.getEleCabPlanchaDTO().getRutaImagen().equalsIgnoreCase("")) {
				try {
	 ...
	 * @throws Exception
	 */
	public ByteArrayOutputStream generarReporteAsociadosHabilesWeb(List<EleAsociadoDatosDTO> listAsociadosHabiles,
			String pathServidor) throws Exception {
		Document reporteAsociado = null;
		ByteArrayOutputStream file = null;
		HeaderFooter header = null;
		try {
			...
			cell = new PdfPCell(new Paragraph(loaderResourceElements
					.getKeyResourceValue(ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteZona"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);

			cell = new PdfPCell(new Paragraph(loaderResourceElements
					.getKeyResourceValue(ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblRegional"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);

			cell = new PdfPCell(new Paragraph(loaderResourceElements.getKeyResourceValue(
					ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteOficina"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);
			...
			cell = new PdfPCell(
					new Paragraph(
							loaderResourceElements.getKeyResourceValue(
									ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteNombreCompleto"),
							font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);

			cell = new PdfPCell(new Paragraph(loaderResourceElements
					.getKeyResourceValue(ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteCiudad"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);

			cell = new PdfPCell(new Paragraph(loaderResourceElements.getKeyResourceValue(
					ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteProfesion"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);

			cell = new PdfPCell(new Paragraph(loaderResourceElements.getKeyResourceValue(
					ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteFechaNac"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);

			cell = new PdfPCell(new Paragraph(loaderResourceElements.getKeyResourceValue(
					ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteFechaVin"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);

			cell = new PdfPCell(new Paragraph(loaderResourceElements.getKeyResourceValue(
					ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteEstrato"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);

			cell = new PdfPCell(new Paragraph(loaderResourceElements
					.getKeyResourceValue(ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteGenero"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);

			cell = new PdfPCell(new Paragraph(loaderResourceElements
					.getKeyResourceValue(ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteEmail"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);

			cell = new PdfPCell(new Paragraph(loaderResourceElements.getKeyResourceValue(
					ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteDireccion"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);

			cell = new PdfPCell(new Paragraph(loaderResourceElements.getKeyResourceValue(
					ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteTelefono"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);

			cell = new PdfPCell(new Paragraph(loaderResourceElements.getKeyResourceValue(
					ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteCelular"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);
	 ...
	 * @throws Exception
	 */
	public ByteArrayOutputStream generarReporteAsociadosHabilesInhabilesPDF(
			List<EleAsociadoDatosDTO> listAsociadosHabiles, String pathServidor) throws Exception {
		Document reporteAsociado = null;
		ByteArrayOutputStream file = null;
		HeaderFooter header = null;
		try {
			...
			for (EleAsociadoDatosDTO dto : listAsociadosHabiles) {
				if (c == 1) {
					...
					cell = new PdfPCell(
							new Paragraph(
									loaderResourceElements.getKeyResourceValue(
											ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreportePeriodo"),
									font));
					cell.setBackgroundColor(HdColor);
					table.addCell(cell);

					cell = new PdfPCell(new Paragraph(loaderResourceElements
							.getKeyResourceValue(ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblZona"), font));
					cell.setBackgroundColor(HdColor);
					table.addCell(cell);
					...
					cell = new PdfPCell(
							new Paragraph(
									loaderResourceElements.getKeyResourceValue(
											ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteProfesion"),
									font));
					cell.setBackgroundColor(HdColor);
					table.addCell(cell);
					...
					cell = new PdfPCell(new Paragraph(loaderResourceElements.getKeyResourceValue(
							ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblRegional"), font));
					cell.setBackgroundColor(HdColor);
					table.addCell(cell);
					...
					cell = new PdfPCell(
							new Paragraph(
									loaderResourceElements.getKeyResourceValue(
											ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteZona"),
									font));
					cell.setBackgroundColor(HdColor);
					table.addCell(cell);
					...
					cell = new PdfPCell(
							new Paragraph(
									loaderResourceElements.getKeyResourceValue(
											ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteOficina"),
									font));
					cell.setBackgroundColor(HdColor);
					table.addCell(cell);
				}
	...
	/**
	 * Metodo que genera el reporte de novedades aplicadas filtrado por novedad,
	 * zona y fecha de proceso.
	 * 
	 * @author <a href="mailto:bernando.lopez@pragma.com.co">Bernardo López</a> -
	 *         Pragma S.A. <br>
	 * @date 20/11/2012
	 ...
	 * @throws Exception
	 */
	public ByteArrayOutputStream generarReporteNovedadesAplicadasPDF(List<EleNovedadDTO> list, String pathServidor)
			throws Exception {
		Document reporteAsociado = null;
		ByteArrayOutputStream file = null;
		HeaderFooter header = null;
		try {
			...
			cell = new PdfPCell(
					new Paragraph(
							loaderResourceElements.getKeyResourceValue(
									ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblreporteNumeroCedula"),
							font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);
			...
			cell = new PdfPCell(new Paragraph(loaderResourceElements.getKeyResourceValue(
					ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblNovedadAplicada"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);
			...
			cell = new PdfPCell(
					new Paragraph(
							loaderResourceElements.getKeyResourceValue(
									ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblReporteFechaNovedad"),
							font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);
			...
			cell = new PdfPCell(new Paragraph(loaderResourceElements
					.getKeyResourceValue(ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "lblZonaElectoral"), font));
			cell.setBackgroundColor(HdColor);
			table.addCell(cell);
	 ...
	 * @throws Exception
	 */
	public ByteArrayOutputStream generarReporteResumenHabilidadesPDF(List<ResumenHabilidadDTO> list,
			String pathServidor) throws Exception {
		Document reporteAsociado = null;
		ByteArrayOutputStream file = null;
		HeaderFooter header = null;
	 ...
	 * @throws Exception
	 */
	public ByteArrayOutputStream generarReporteResumenNovedadesPDF(List<ResumenNovedadesDTO> list, String pathServidor)
			throws Exception {
		Document reporteAsociado = null;
		ByteArrayOutputStream file = null;
		HeaderFooter header = null;
