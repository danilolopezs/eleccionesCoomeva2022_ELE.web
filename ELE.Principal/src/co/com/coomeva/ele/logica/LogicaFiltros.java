...
public class LogicaFiltros {
	...
	public List<FiltrosConsultasDTO> consultarZonas() throws Exception {
		...
		try {
			...
			throw new Exception(loaderResourceElements.getKeyResourceValue(
					ConstantesProperties.NOMBRE_ARCHIVO_MENSAJES_LOGICA_PRINCIPAL, "consulta.zonas"));
		} finally {
			if(session != null) {
				session.close();
			}
	...
	public List<FiltrosConsultasDTO> consultarCiudades() throws Exception {
		...
		try {
			...
			throw new Exception(loaderResourceElements.getKeyResourceValue(
					ConstantesProperties.NOMBRE_ARCHIVO_MENSAJES_LOGICA_PRINCIPAL, "consulta.ciudades"));
		} finally {
			if(session != null) {
				session.close();
			}
	...
	public List<FiltrosConsultasDTO> consultarRegionales() throws Exception {
		...
		try {
			...
			if (elements != null && elements.size() > 0) {
				 ...
				 */
				for (Object[] object : elements) {
					// Se modifica para que únicamente me aparezca el nombre de la regional sin el
					// Reg.
					...
					// if
					// (regional.equals(loaderResourceElements.getKeyResourceValue(ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB,
	...
	public List<FiltrosConsultasDTO> consultarZonasPorRegional(Long codRegional) throws Exception {
		...
		FiltrosConsultasDTO dto = null;
		Session session = HibernateSessionFactoryElecciones2012.getSession();
		String cadenaZonaEle = loaderResourceElements.getKeyResourceValue(
				ConstantesProperties.NOMBRE_ARCHIVO_PARAMETROS_PRINCIPAL, ConstantesProperties.CADENA_ZONA_NUM),
				cadenaZonaEleEspecial = loaderResourceElements.getKeyResourceValue(
						ConstantesProperties.NOMBRE_ARCHIVO_PARAMETROS_PRINCIPAL, ConstantesProperties.CADENA_ZONA_NUM);
			...
			if (elements != null && elements.size() > 0) {
				for (Object[] object : elements) {
					...
					if ((Long) object[0] > 28l) {
						dto.setDescripcionFiltro(
								cadenaZonaEleEspecial + object[2].toString() + " " + object[1].toString());
					} else {
	...
	public List<FiltrosConsultasDTO> consultarZonasElectorales() throws Exception {
		...
		try {
			...
			throw new Exception(loaderResourceElements.getKeyResourceValue(
					ConstantesProperties.NOMBRE_ARCHIVO_MENSAJES_LOGICA_PRINCIPAL, "consulta.zonas.electorales"));
		} finally {
			if(session != null) {
				session.close();
			}
	...
	public FiltrosConsultasDTO consultarZonaElectoralePorAsociado(Long numeroDocumento) throws Exception {
		...
		String cadenaZonaEle = loaderResourceElements.getKeyResourceValue(
				ConstantesProperties.NOMBRE_ARCHIVO_PARAMETROS_PRINCIPAL, ConstantesProperties.CADENA_ZONA_ELECTORAL),
				cadenaZonaEleEspecial = loaderResourceElements.getKeyResourceValue(
		...
		try {
			...
			if (element != null) {
				...
				if ((Long) element[0] > 28l) {
					dto.setDescripcionFiltro(
							cadenaZonaEleEspecial + element[2].toString() + " " + element[1].toString());
				} else {
	 ...
	 * @throws Exception
	 */
	public FiltrosConsultasDTO consultarZonaElectoralEspePorEmpleado(Long numeroDocumento) throws Exception {
		FiltrosConsultasDTO dto = new FiltrosConsultasDTO();
		Object[] element = null;
		Session session = null;
		String cadenaZonaEle = loaderResourceElements.getKeyResourceValue(
