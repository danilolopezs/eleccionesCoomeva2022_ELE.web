...
public class LogicaAutenticacion {
	...
	//*public static Parametro urlP = LogicaElePParametro.getInstance().getParametroFuenteP("propiedades_pm", "url_ws_pm");
	public static String urlS = UtilAcceso.getParametroFuenteS("parametros", "COD_PARAM_URL_WS_PM");

	//public static Parametro nombreAplicacionP = LogicaElePParametro.getInstance().getParametroFuenteP("propiedades_pm", "nombreAplicacion");
	public static String nombreAplicacionS = UtilAcceso.getParametroFuenteS("parametros", "COD_PARAM_NOMBRE_APLICACION");

	//public static Parametro ldapP = LogicaElePParametro.getInstance().getParametroFuenteP("propiedades_pm", "fuente_ldap");
	public static String ldapS = UtilAcceso.getParametroFuenteS("parametros", "COD_PARAM_FUENTE_LDAP");

	//public static Parametro daP = LogicaElePParametro.getInstance().getParametroFuenteP("propiedades_pm", "fuente_da");
	...
	try {
		...
		if (tipoVal != null)
		{
			...
			//userVo = autenticacion.validarUsuario(login, password, urlS, tipoVal, nombreAplicacionS, 0);
		...
		//				throw new Exception(UtilAcceso.getParametroFuenteS("mensajes","noUsuarioValido"));
		//			}
	} catch (Exception e) {
		throw e;
	}
	...
	/*
		...
		CaasStub caasStub = null;
		ValidateUser valUser = null;
		try {

			caasStub = new CaasStub(urlS);
			valUser = new ValidateUser();
	 ...
	 * @throws Exception 
	 */
	public UserVo autenDirectorioOpenLDAP(String login, String password) throws Exception{		if(login == null || login.equals(""))
		throw new Exception(UtilAcceso.getParametroFuenteS("mensajes", "noUsuario"));
		if (password == null || password.equals("")) {
			throw new Exception(UtilAcceso.getParametroFuenteS("mensajes", "noPassword"));
		}	
