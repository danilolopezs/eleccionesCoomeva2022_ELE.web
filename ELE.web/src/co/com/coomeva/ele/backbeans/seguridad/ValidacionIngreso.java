...
public class ValidacionIngreso extends BaseVista {
	...
	/**
	 * Metodo que obtiene el numero de documento y verifica que el asociado
	 * exista y que si es o no cabeza de plancha
	 * 
	...
	public String action_ingreso() {
		...
		try {
			...
			/**
			 * Se valida que el usuario ingrese el número de identificación a
			 * validar
			 */
			if (documento == null) {
				throw new EleccionesDelegadosException(UtilAcceso.getParametroFuenteS("mensajes",
						"noCedulaValidacion"));
			}
			...
			if (tipoEleccionesRepresentantes.equals(tipoEleccionesSession)
					&& DelegadoAsociado.getInstance()
							.esAsociadoPersonaJuridica(documento)) {
			...
			if (DelegadoAsociado.getInstance().esAsociado(documento)) {
				...
				if (DelegadoAsociado.getInstance().estaAsociaActivo(documento)) {
					if (DelegadoAsociado.getInstance()
							.asociadoPerteneceZonaHayEleccion(documento)) {
						asociado = DelegadoAsociado.getInstance()
								...
								MessageFormat
										.format(
												UtilAcceso
														.getParametroFuenteS(
																"mensajes",
																"msgAsociadoNoPerteneceAzonaElectoral"),
												documento.toString()));
					}
			...
			/**
			 * se valida que la fecha actual sea una fecha hábil para el proceso
			...
			ParametroPlanchaDTO parametroFechaInicial = LectorParametros
					.obtenerParametrosCodigoTipo(UtilAcceso
							.getParametroFuenteL("parametros",
									"campo.param.registro.inicio"), UtilAcceso
							.getParametroFuenteL("parametros",
									"campo.param.registro.tipo"));
			ParametroPlanchaDTO parametroFechaFinal = LectorParametros
					.obtenerParametrosCodigoTipo(UtilAcceso
							.getParametroFuenteL("parametros",
									"campo.param.registro.fin"), UtilAcceso
							.getParametroFuenteL("parametros",
									"campo.param.registro.tipo"));

			Date dateFechaIniInscrpcion = ManipulacionFechas.stringToDate(
					parametroFechaInicial.getStrValor(), "yyyy-MM-dd hh:mm:ss");
			Date dateFechaFinInscrpcion = ManipulacionFechas.stringToDate(
			...
			if (habilidad) {
				DelegadoLogAsociado.getInstance().crearRegistroLogAsociado(
						null,
						UtilAcceso.getParametroFuenteS("parametros",
								"tipTrn_autenticacion"),
						ObtenerIP.getInstance().getIP(), null,
						String.valueOf(documento), null);
				...
				throw new EleccionesDelegadosException(UtilAcceso.getParametroFuenteS("mensajes",
						"fechaFueraRango")
						+ ": ["
						+ parametroFechaInicial.getStrValor()
						+ "  -  "
						+ parametroFechaFinal.getStrValor() + "]");
			}
			 }else{
				 throw new EleccionesDelegadosException(UtilAcceso.getParametroFuenteS("mensajes",
				 "noHabil2"));
	...
	public DTOHabilidadAsociado getAsociado() {
		if (asociado == null) {
			asociado = (DTOHabilidadAsociado) FacesUtils
					.getSessionParameter("asociado");
		}
		return asociado;
	}
