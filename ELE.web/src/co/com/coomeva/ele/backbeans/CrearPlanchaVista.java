	...
	private void init() {
		try {
			...
			if (planchaDTO != null) {
				...
				String separador[] = planchaDTO.getEleCabPlanchaDTO().getOtrosEstudios()
...
//				listaDocumentos.add(eleTablaCabezaPlancha);
	...
	/**
	 * Metodo que oculta los PopUps de carga de imagen y restricciones de las
	 * imagenes
	 * 
	 * @author Manuel Galvez, Ricardo Chiriboga
	 * @return String
	...
	public void uploadFile(ActionEvent event) throws Exception {
		...
		try {
			...
			if (size < UtilAcceso.getParametroFuenteI("parametros", "minImage")
					|| size > UtilAcceso.getParametroFuenteI("parametros", "maxImage")) {
			...
			if (fileInfo.getStatus() == FileInfo.SAVED) {
				...
				File archivoAnterior = new File(currentFile.getFile().getParentFile().getAbsolutePath() + "//" + zona2
						+ noPlancha + "_" + nombreImagen + ".jpg");

				if (archivoAnterior.exists()) {
					archivoAnterior.delete();
					currentFile.getFile().renameTo(new File(currentFile.getFile().getParentFile().getAbsolutePath()
							+ "//" + zona2 + noPlancha + "_" + nombreImagen + ".jpg"));
				} else
					currentFile.getFile().renameTo(new File(currentFile.getFile().getParentFile().getAbsolutePath()
							+ "//" + zona2 + noPlancha + "_" + nombreImagen + ".jpg"));
				...
				imagenCabPlancha = UtilAcceso.getParametroFuenteS("parametros", "directorioImagenesAso") + zona2
						+ noPlancha + "_" + nombreImagen + ".jpg";
...
//		}
		...
		for (EleTablaDocumentosDTO eleTablaDocumentos : listaDocumentos) {
			if (eleTablaDocumentos.getCedulaPrincipal() != null
					&& !eleTablaDocumentos.getCedulaPrincipal().trim().equals("")) {
				if (!eleTablaDocumentos.getCedulaPrincipal().equalsIgnoreCase("")) {
					elePrincipalesDTO = new ElePrincipalesDTO();
					elePrincipalesDTO.setNroPriIdentificacion(eleTablaDocumentos.getCedulaPrincipal());
					listaPrincipales.add(elePrincipalesDTO);
			...
			if (eleTablaDocumentos.getCedulaSuplente() != null
					&& !eleTablaDocumentos.getCedulaSuplente().trim().equals("")) {
				if (!eleTablaDocumentos.getCedulaSuplente().equalsIgnoreCase("")) {
					eleSuplentesDTO = new EleSuplentesDTO();
					eleSuplentesDTO.setNroSuIdentificacion(eleTablaDocumentos.getCedulaSuplente());
					listaSuplentes.add(eleSuplentesDTO);
		...
		try {
			...
			for (EleExperienciaLaboral eleExperienciaLaboral : listExperienciaLaboral) {
				if (eleExperienciaLaboral.getEmpresa() != null
						&& !eleExperienciaLaboral.getEmpresa().equalsIgnoreCase("")) {
					if (eleExperienciaLaboral.getCargo() != null
							&& !eleExperienciaLaboral.getCargo().equalsIgnoreCase("")) {
			...
			cabezaPlancha.setOtrosEstudios(
					estudios1 + UtilAcceso.getParametroFuenteS("parametros", "separadorEstudios") + estudios2);
			...
			for (ElePrincipalesDTO principalesDTO : listaPrincipales) {
				elePrincipales = DelegadoPrincipal.getInstance()
						.consultarPrincipal(principalesDTO.getNroPriIdentificacion());
				eleCabPlancha = DelegadoCabezaPlancha.getInstance()
						.consultarCabezaPlancha(principalesDTO.getNroPriIdentificacion());
				eleSuplentes = DelegadoSuplente.getInstance()
						.consultarSuplente(principalesDTO.getNroPriIdentificacion());

				if (elePrincipales != null) {
					throw new Exception(UtilAcceso.getParametroFuenteS("mensajes", "msgExisteIdPrincipal") + " " + "''"
							+ principalesDTO.getNroPriIdentificacion() + "''");
				}
				if (eleCabPlancha != null) {
					throw new Exception(UtilAcceso.getParametroFuenteS("mensajes", "msgExisteIdCabPlancha") + " " + "''"
							+ principalesDTO.getNroPriIdentificacion() + "''");
				}
				if (eleSuplentes != null) {
					throw new Exception(UtilAcceso.getParametroFuenteS("mensajes", "msgExisteIdSuplente") + " " + "''"
							+ principalesDTO.getNroPriIdentificacion() + "''");
				}
			}
			...
			for (EleSuplentesDTO suplentesDTO : listaSuplentes) {
				...
				eleCabPlancha = DelegadoCabezaPlancha.getInstance()
						.consultarCabezaPlancha(suplentesDTO.getNroSuIdentificacion());
				elePrincipales = DelegadoPrincipal.getInstance()
						.consultarPrincipal(suplentesDTO.getNroSuIdentificacion());

				if (eleSuplentes != null) {
					throw new Exception(UtilAcceso.getParametroFuenteS("mensajes", "msgExisteIdSuplente") + " " + "''"
							+ suplentesDTO.getNroSuIdentificacion() + "''");
				}
				if (eleCabPlancha != null) {
					throw new Exception(UtilAcceso.getParametroFuenteS("mensajes", "msgExisteIdCabPlancha") + " " + "''"
							+ suplentesDTO.getNroSuIdentificacion() + "''");
