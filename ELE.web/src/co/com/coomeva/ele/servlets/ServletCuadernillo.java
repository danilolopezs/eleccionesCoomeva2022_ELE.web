...
public class ServletCuadernillo extends HttpServlet{
	...
	public synchronized  void performTask(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		ElePlanchaDTO plancha = (ElePlanchaDTO)session.getAttribute("planchaImprimir");
		boolean flag = false;
		
		try {
			if (plancha==null) {
				flag = true;
				throw new Exception(UtilAcceso.getParametroFuenteS("mensajes", "noPlanchaImprimir"));	
			}
			byteArrayOutputStream = DelegadoReportes.getInstance().generarCuadernillo(plancha,PathRequest.getInstance().getPathServerContextPath(getServletContext(), ""));
			//byteArrayOutputStream = DelegadoReportes.getInstance().generarCuadernillo2(new ElePlanchaDTO(),PathRequest.getInstance().getPathServerContextPath(getServletContext(), "imagenes"));
			
			session.removeAttribute("planchaImprimir");
