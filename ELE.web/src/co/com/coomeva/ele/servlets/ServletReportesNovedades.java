...
public class ServletReportesNovedades extends HttpServlet {
	...
	public void executeReport(HttpServletRequest req,HttpServletResponse resp) throws Exception {
		...
		String nombreArchivo = LoaderResourceElements.getInstance().getKeyResourceValue(ConstantesProperties.NOMBRE_ARCHIVO_ETIQUETAS_WEB, "nombreReporteNovedades");
		...
		try {
			...
			/*if (fechaProceso != null) {
				fechaCorte = DelegadoProceso.getInstance().consultaFechaCorteProcesoPorCodigo(fechaProceso);
			}*/
			
			List<EleNovedadDTO> listNovedades = DelegadoNovedad.getInstance().consultaNovedaesAplicadas(novedad, fechaCorte, zonaElectoral);
			...
			if (tipoReporte.equals(LoaderResourceElements.getInstance().getKeyResourceValue(ConstantesProperties.NOMBRE_ARCHIVO_PARAMETROS_PRINCIPAL, "tipoReporteHabilidadExcel"))) {
				...
				byteArrayOutputStream =  DelegadoReportes.getInstance().generarReporteNovedadesAplicadasPDF(listNovedades, 
