...
public class ServletReportePlanchasEstado extends HttpServlet {
	 ...
	 */
	public ServletReportePlanchasEstado() {
		super();
	}
	...
	public void executeReport(HttpServletRequest req,HttpServletResponse resp) throws Exception {
		...
		String nombreArchivo = "ReportePlanchasEstado";
		
		try {
			UserVo user  = (UserVo)req.getSession().getAttribute("user");
			Long estado = new Long(req.getSession().getAttribute("estado").toString());
			List<PlanchaPorEstadoDTO> list = DelegadoPlanchas.getInstance().obtenerPlanchasPorEstado(estado, user.getUserId());
			
			if (list == null || list.size() <= 0) {
				flag = true;
				
				//PROBAR CUAL ES EL MENSAJE QUE SE MUESTRA
				throw new Exception(UtilAcceso.getParametroFuenteS("mensajes", "msgNoDatosBusqueda"));
			}
			
			
			byteArrayOutputStream = DelegadoGenerador.getInstance().generarReportePlanchasEstado(list);
			resp.setContentType("application/vnd.ms-excel");
			resp.setHeader("Content-disposition", "attachment; filename=\""+nombreArchivo+".xls\"");													
