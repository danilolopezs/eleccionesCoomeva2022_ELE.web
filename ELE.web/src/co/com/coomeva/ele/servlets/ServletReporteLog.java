...
public class ServletReporteLog extends HttpServlet{
	...
	public synchronized  void performTask(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		try {
			List<LogTransacciones> listaTransacciones  = DelegadoLogHab.getInstance().consultarLogs(identificacion, fechaInicial, fechaFinal, estado, usuario);
			if (listaTransacciones.size()==0) {
				flag = true;
				throw new Exception(UtilAcceso.getParametroFuenteS("mensajes", "msgNoDatosBusqueda"));	
			}
			byteArrayOutputStream = DelegadoReportes.getInstance().generarTransaccionesByte(listaTransacciones,PathRequest.getInstance().getPathServerContextPath(getServletContext(), "imagenes"));
