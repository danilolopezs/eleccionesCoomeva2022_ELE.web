...
public class EleAsociadoDAO implements IEleAsociadoDAO {
	...
	// public static final String DIRECCIONCORRESPONDENCIA =
	...
	/**
	 *
	 * @param Instance EleAsociado Instance to persist
	...
	/**
	 * @param Instance EleAsociado Instance to delete
	 * @throws RuntimeException when the operation fails
	 */
	...
	/**
	 *
	 * @param Instance EleAsociado Instance to update
	 * @throws RuntimeException when the operation fails
	 */
	...
	public EleAsociado findById(Long id) {
		...
		try {
			EleAsociado instance = (EleAsociado) getSession().get(EleAsociado.class, id);
	...
	public List<EleAsociado> findByExample(EleAsociado instance) {
		...
		try {
			List results = getSession().createCriteria("EleAsociado").add(Example.create(instance)).list();
	...
	public List<EleAsociado> findByProperty(String propertyName, Object value) {
		log.debug("finding EleAsociado instance with property: " + propertyName + ", value: " + value);

		try {
			String queryString = "FROM EleAsociado as model where model." + propertyName + "= ?";
	...
	public List<EleAsociado> findByDireccionCorrespondencia(Object direccionCorrespondencia) {
	...
	public List<EleAsociado> findByCriteria(String whereCondition) {
		...
		try {
			...
			final String queryString = "select model from EleAsociado model " + where;
	...
	public List<EleAsociado> findPageEleAsociado(String sortColumnName, boolean sortAscending, int startRow,
			int maxResults) {
		log.debug("finding EleAsociado findPageEleAsociado");

		if ((sortColumnName != null) && (sortColumnName.length() > 0)) {
			try {
				String queryString = "select model from EleAsociado model order by model." + sortColumnName + " "
