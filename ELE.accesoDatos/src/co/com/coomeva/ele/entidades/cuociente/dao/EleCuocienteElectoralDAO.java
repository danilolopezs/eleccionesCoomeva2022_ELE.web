 ...
 * EleCuocienteElectoral entities. Transaction control of the save(), update()
...
public class EleCuocienteElectoralDAO extends BaseHibernateDAO {
	...
	public List findByExample(EleCuocienteElectoral instance) {
		...
		try {
			List results = getSession().createCriteria("co.com.coomeva.ele.entidades.cuociente.EleCuocienteElectoral")
	...
	public EleCuocienteElectoral merge(EleCuocienteElectoral detachedInstance) {
		...
		try {
			EleCuocienteElectoral result = (EleCuocienteElectoral) getSession().merge(detachedInstance);
